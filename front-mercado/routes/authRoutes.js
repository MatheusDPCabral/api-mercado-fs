...
router.post('/login', (req, res) => {
  ...
  if (user && bcrypt.compareSync(password, user.password)) {
    ...
    return res.redirect('/products'); // Redirecionar para a página de produtos após o login
  }

  // Se chegou aqui, significa que as credenciais estão incorretas

  // Verificar se ambos email e senha estão incorretos
  if (!user && password) {
    emailError = 'Credenciais inválidas';
    passwordError = 'Credenciais inválidas';
  }

  // Se o usuário foi encontrado, mas a senha está incorreta
  if (user && !bcrypt.compareSync(password, user.password)) {
    passwordError = 'Credenciais inválidas';
  }

  // Renderizar a página de login novamente com os erros e valores preenchidos
  res.render('login', {
    email,
    password,
    emailError,
